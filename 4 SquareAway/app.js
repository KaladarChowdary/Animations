...
let cordArray, sqrArray, size, gap, range;
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// EVENT LISTENERS
...
window.addEventListener("resize", function () {
  ...
  sqrArray = createSquareArray(size, gap);
});
...
// CLASSES
class Square {
  constructor(x = middleX(), y = middleY(), size = 10, color = "white") {
    this.originalX = x;
    this.originalY = y;
    this.x = x;
    this.y = y;
  ...
  draw() {
    fillRectangle(this.x, this.y, this.size, this.size, this.color, this.color);
  }
  ...
  withinRange() {
    return this.dFromMouse() < this.rge;
  }

  moveIfInRange() {
    while (this.withinRange()) {
      this.x += this.moveDirection();
      this.y += this.moveDirection();
    }
  }

  moveDirection() {
    if (this.x + this.size / 2 < mouse.x) {
      return -this.awaySpeed;
    } else {
      return this.awaySpeed;
    }
  }
  ...
  settleDown() {
    if (!this.withinRange()) {
      if (this.x < this.originalX) {
        this.x += this.closeSpeed;
        this.x = Math.min(this.x, this.originalX);
      }
      if (this.x > this.originalX) {
        this.x -= this.closeSpeed;
        this.x = Math.max(this.x, this.originalX);
      }

      if (this.y < this.originalY) {
        this.y += this.closeSpeed;
        this.y = Math.min(this.y, this.originalY);
      }

      if (this.y > this.originalY) {
        this.y -= this.closeSpeed;
        this.y = Math.max(this.y, this.originalY);
      }
    }
  }
}
...
function createSquareArray(size, gap) {
  let cordArray = giveCoordinatesArray(size, gap);
  let sqr = [];
  for (let [x, y] of cordArray) {
    sqr.push(new Square(x, y, size, "white"));
  }
  return sqr;
}
