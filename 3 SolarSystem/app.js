...
let no, radius, stars, sun, draggable;
let planets, colorArray, sizeArray, distanceArray, speedArray;
let mercury, venus, earth, mars, jupiter, saturn, uranus, nepute;
colorArray = [
  "pink",
  "#FFFFE0",
  "#ADD8E6",
...
sizeArray = [0.5, 0.8, 1, 0.7, 3, 2.5, 2, 1.5];
distanceArray = [0.4, 0.7, 1, 1.3, 1.5, 1.7, 1.9, 2.3];
speedArray = [47, 35, 28, 24, 13, 9, 6, 5];
...
window.addEventListener("resize", function () {
  maxify();
  stars = objectsOfClass(Star, no);
  sun = new Ball(middleX(), middleY(), 30, "yellow");
  planets = [];
  ...
  moon = new RevolvingBall(planets[2], 20, 0.1, 2, "white");
});

window.addEventListener("mousedown", function (evt) {
  if (getDistance(evt.pageX, evt.pageY, sun.x, sun.y) < sun.radius) {
    draggable = true;
  }
});
...
class Star {
  constructor() {
    this.radius = randRange(0.01, 0.1);
    this.x = getAcceptableX(this.radius);
    this.y = getAcceptableY(this.radius);
  }

  draw() {
    drawBall(this.x, this.y, this.radius, 1, "white", "white");
  }
  update() {
    this.draw();
  }
...
class Ball {
  constructor(x = middleX(), y = middleY(), radius = 20, color = " #ffff00") {
    this.x = x;
    this.y = y;
    this.radius = radius;
    this.color = color;
...
class RevolvingBall {
  constructor(
    ...
    this.theta = randRange(0, 2 * Math.PI);
    this.updateXandY();
  ...
  updateXandY() {
    this.x = newX(this.master.x, this.master.y, this.distance, this.theta);
    this.y = newY(this.master.x, this.master.y, this.distance, this.theta);
  }
